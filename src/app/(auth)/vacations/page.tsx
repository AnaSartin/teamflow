          ...
          <div key={key} className="mb-4">
            ...
            <div className="space-y-1.5">
              {vacList.map(v => {
                const c = v.collaborators
                return (
                  <Link key={v.id} href={`/collaborators/${c?.id}`}
                    className="flex items-center gap-3 bg-white rounded-lg border border-slate-200 px-4 py-2.5 hover:bg-slate-50 transition-colors"
                  >
                    <Avatar name={c?.name ?? ''} size="sm" />
                    ...
                    <Badge variant={differenceInDays(parseISO(v.expiry_date), today) < 90 ? 'amber' : 'green'}>
